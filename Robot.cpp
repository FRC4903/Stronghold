...
//			hold_val = -1.0;
//		}
//		if (action_control->GetRawButton(2) && !intake_sensor->Get()) {
//			hold_val = 1.0;
//			intake_val = 1.0;
//		}
//
...
private:
	...
	void RobotInit()
	{
		...
		// Camera stuff
		leCam = new USBCamera("cam0", false);
		leCam->OpenCamera();
		leCam->StartCapture();
		leCam->SetExposureManual(1);
		leCam->SetBrightness(1);
		leCam->SetWhiteBalanceManual(5200);
		screen = imaqCreateImage(IMAQ_IMAGE_RGB, 0);
	...
	void TeleopPeriodic()
	{
		...
		leCam->GetImage(screen);
		CameraServer::GetInstance()->SetImage(screen);
